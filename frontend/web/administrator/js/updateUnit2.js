...
//     };
// }]);
app.controller('UpdateUnitCtrl2', ['$scope', '$element', '$http', '$window', function ($scope, $element, $http, $window) {
    ...
    $http({
        method: 'GET',
        url: backendURI + '/administrator/unit/dimension-api',
    })
        .then(function (response) {
    ...
    $scope.submitForm = function (unit_id) {
        ...
        $http({
            method: "POST",
            url: backendURI + "/administrator/unit/update2?unit_id=" + unit_id,
            data: $scope.formData
        }).then(function (response) {
